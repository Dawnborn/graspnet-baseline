...
at::Tensor cylinder_query(at::Tensor new_xyz, at::Tensor xyz, at::Tensor rot,
                          const float radius, const float hmin,
                          const float hmax, const int nsample) {
  ...
  at::Tensor idx =
      torch::zeros({new_xyz.size(0), new_xyz.size(1), nsample},  // 1 1024 64
                   at::device(new_xyz.device()).dtype(at::ScalarType::Int));

  if (new_xyz.type().is_cuda()) {
    query_cylinder_point_kernel_wrapper(
        xyz.size(0), xyz.size(1), new_xyz.size(1), radius, hmin, hmax, nsample,
        new_xyz.data<float>(), xyz.data<float>(), rot.data<float>(),
        idx.data<int>());
  } else {
